...
namespace arg {
     ...
     */
    struct QueryFilter {
        explicit QueryFilter(CLI::App* app) : m_app(app) {}

        auto query_filter_add_options() {
            m_app->add_option("--min-query-len", m_min_length, "Minimum query length");
            m_app->add_option("--max-query-len", m_max_length, "Maximum query length");
        }
